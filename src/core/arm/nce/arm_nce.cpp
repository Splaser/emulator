...
bool ArmNce::HandleFailedGuestFault(GuestContext* guest_ctx, void* raw_info, void* raw_context) {
    ...
    if (!is_prefetch_abort) {
        ...
        if (!parent.m_logged_unhandled_data_abort.exchange(true, std::memory_order_relaxed)) {
            ...
            LOG_CRITICAL(Core_ARM,
                         "NCE data abort registers: x0={:#x}, x1={:#x}, x2={:#x}, x3={:#x}, "
                         "x4={:#x}, x5={:#x}, x6={:#x}, x7={:#x}",
            ...
            LOG_CRITICAL(Core_ARM,
                         "NCE data abort registers: x8={:#x}, x9={:#x}, x10={:#x}, x11={:#x}, "
            ...
            LOG_CRITICAL(Core_ARM,
                         "NCE data abort registers: x16={:#x}, x17={:#x}, x18={:#x}, x19={:#x}, "
            ...
            LOG_CRITICAL(Core_ARM,
                         "NCE data abort registers: x24={:#x}, x25={:#x}, x26={:#x}, x27={:#x}, "
                         "x28={:#x}, x29={:#x}, x30={:#x}",
                         host_ctx.regs[24], host_ctx.regs[25], host_ctx.regs[26], host_ctx.regs[27],
                         host_ctx.regs[28], host_ctx.regs[29], host_ctx.regs[30]);
            const u64 x0 = host_ctx.regs[0];
            LOG_CRITICAL(Core_ARM,
                         "NCE data abort x0 packed-fence candidate: id={}, value={:#x}, "
                         "packed={:#018x}, fault_delta={:#x}",
                         static_cast<s32>(x0), static_cast<u32>(x0 >> 32), x0,
                         static_cast<s64>(reinterpret_cast<u64>(info->si_addr)) -
                             static_cast<s64>(x0));
            Service::Nvidia::LogRecentNvFenceTrace();
        }
    ...
    const auto failed_abort =
        std::find_if(parent.m_failed_data_aborts.begin(), parent.m_failed_data_aborts.end(),
                     [&](const FailedDataAbort& candidate) {
                         return host_ctx.pc == 0 && candidate.lr >= 0x1000 && candidate.x0 != 0 &&
